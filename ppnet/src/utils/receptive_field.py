...
'''
The receptive field of a neuron in a CNN is the region of the input image 
that affects the neuron's activation. Deeper layers have larger receptive 
fields, meaning they capture more contextual information.
'''

def compute_rf_protoL_at_spatial_location(image_size, height_index, width_index, protoL_rf_info):
    n, j, r, start = protoL_rf_info[0:4]
    
    # •	n → Grid size of the prototype layer.
	# •	j → Stride (jump size) between prototypes.
	# •	r → Receptive field size (diameter).
	# •	start → Start position of the receptive field.
    ...
    rf_end_height_index = min(int(center_h + (r/2)), image_size)

    rf_start_width_index = max(int(center_w - (r/2)), 0)
    rf_end_width_index = min(int(center_w + (r/2)), image_size)

    return [rf_start_height_index, rf_end_height_index, rf_start_width_index, rf_end_width_index]

def compute_rf_prototype(image_size, prototype_patch_index, protoL_rf_info):
    img_index, height_index, width_index = prototype_patch_index[0:3]
    rf_indices = compute_rf_protoL_at_spatial_location(image_size,
                                                       height_index,
                                                       width_index,
                                                       protoL_rf_info)
    return [img_index, *rf_indices[0:4]]

def compute_rf_prototypes(image_size, prototype_patch_indices, protoL_rf_info):
    rf_prototypes = []
    for prototype_patch_index in prototype_patch_indices:
        ...
        rf_indices = compute_rf_protoL_at_spatial_location(image_size,
                                                           ...
                                                           width_index,
                                                           protoL_rf_info)
        rf_prototypes.append([img_index, *rf_indices[0:4]])
...
def compute_proto_layer_rf_info(image_size, cfg, prototype_kernel_size):
    rf_info = [image_size, 1, 1, 0.5]
...
def compute_proto_layer_rf_info_v2(image_size, layer_filter_sizes, layer_strides, layer_paddings, prototype_kernel_size):
